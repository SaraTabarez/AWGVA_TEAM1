...
<%@ taglib prefix="c" uri="jakarta.tags.core" %><c:set var="ctx" value="${pageContext.request.contextPath}"/>
<!doctype html><html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Detalle de solicitud - Director</title><link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.1/font/bootstrap-icons.css" rel="stylesheet"><link href="${ctx}/assets/css/workflow.css" rel="stylesheet"></head><body>
<jsp:include page="/Layout/sidebar.jsp"/><main class="workflow-main"><div class="detail-head"><div><h1 class="workflow-title">Solicitud de Visita Industrial</h1><div class="workflow-subtitle" style="margin:0">División Académica <c:out value="${expediente.division}"/></div></div><a class="workflow-btn orange" href="${soloLectura ? ctx.concat('/director/historico') : ctx.concat('/director/solicitudes')}"><i class="bi bi-arrow-left"></i> Atrás</a></div>
<div class="workflow-card"><div class="detail-grid">
    <div class="section-caption">Datos reales de la solicitud</div>
    <div class="detail-field"><label class="workflow-label">ID</label><div class="detail-value"><c:out value="${expediente.idVisita}"/></div></div>
    ...
    <c:if test="${not empty expediente.motivoRechazo}"><div class="detail-field span-4"><label class="workflow-label">Observaciones</label><div class="detail-value"><c:out value="${expediente.motivoRechazo}"/></div></div></c:if>
    <div class="section-caption">Documentos (consulta únicamente)</div>
    <c:forEach var="doc" items="${expediente.documentos}"><div class="detail-field"><label class="workflow-label"><c:out value="${doc.tipoLegible}"/></label><a class="workflow-btn light" target="_blank" href="${ctx}/archivo?id=${doc.idDocumento}"><i class="bi bi-eye"></i> Ver · <c:out value="${doc.estadoLegible}"/></a></div></c:forEach>
